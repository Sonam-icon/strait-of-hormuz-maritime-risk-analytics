...
```text
strait-of-hormuz-maritime-risk-analytics/
│
├── README.md
├── PROJECT_CARD.md
├── requirements.txt
...
│
├── notebooks/
│   └── hormuz_trade_analytics.ipynb
│
├── src/
│   └── hormuz_trade_analytics.py
│
├── sql/
│   └── analytics.sql
│
└── outputs/
    ...
    ├── trade_tier_summary.csv
    ├── payment_rail_summary.csv
    ├── continent_summary.csv
    ├── statistical_tests.csv
    ├── daily_operational_metrics.csv
    ├── daily_operational_metrics_with_anomalies.csv
    ├── model_comparison.csv
    ├── xgb_confusion_matrix.csv
    ├── xgb_feature_importance.csv
    ├── reroute_predictions_test_set.csv
...
Power BI is the next stage of the project.

The generated CSV files in `outputs/` are designed to serve as the analytical layer for the dashboard.

Recommended dashboard pages:

### 1. Executive Risk Monitor
...
- Asset exposure by continent
- Asset exposure by commodity
- Cost composition

### 4. Rerouting Analytics

- Rerouting rate by trade tier
- Rerouting rate by commodity
- Rerouting rate by payment rail
- Model predictions
...
- Matplotlib
- SQL
- Power BI (planned next phase)

---

# Resume Description

**Strait of Hormuz Maritime Trade & Risk Analytics | Python, SQL, XGBoost**

- Analyzed 1,890 simulated vessel-level records across trade tiers, routing decisions, payment rails, insurance costs, delays, fuel penalties and asset exposure to identify patterns in maritime trade friction.
- Performed Python/SQL-oriented EDA and statistical testing to evaluate relationships between trade access, rerouting behavior, transit costs and operational delays.
- Benchmarked Logistic Regression and XGBoost models for simulated vessel rerouting using chronological validation and evaluated performance using F1-score, ROC-AUC, precision and recall.
- Engineered operational risk metrics and applied rolling z-score and Isolation Forest methods to detect anomalous periods in simulated maritime activity.
- Developed analytical outputs for a planned Power BI dashboard covering trade friction, financial exposure, rerouting patterns and operational anomalies.
...
**Data Analytics | Data Science | Supply Chain Analytics | Operations Research | Machine Learning**
